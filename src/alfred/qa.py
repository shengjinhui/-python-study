...
def send_http_request(request_url, request_params, request_headers, request_method):
    if request_method.upper() == "GET":
        if not request_params:
            ...
            request = urllib.request.Request(url + urllib.parse.urlencode(request_params), headers=request_headers)
...
def get_http_result(username, password, param):
    ...
    if not cookie_dict:
        return
    request_url = "http://pre.qapt.mogu-inc.com/api/query-user"

    user_id = query if query.isdigit() else ""
    user_name = query if not query.isdigit() else ""
    request_params = {
        "userId": user_id,
        "userName": user_name,
        "domain": "mogujie",
    ...
    request_headers = dict(common_headers, **additional_headers)
    response = send_http_request(request_url, request_params, request_headers, "GET")
    content = decode_response_content(response)
    data = json.loads(content)
    code = data['code']

    if not data or code != 1001:
        ...
        user_id = data['data']['usersInfo']['userId']
        user_name = data['data']['usersInfo']['uname']
        if query.isdigit():
            print(user_name, end='')
        else:
            print(user_id, end='')
